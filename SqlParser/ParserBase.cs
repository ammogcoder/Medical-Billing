...
namespace Parser
{
	...
	public abstract class ParserBase
	{
        ...
        protected int ParseBlock(XmlNode parentNode, TagBase parentTag, string text, int position)
        {
            ...
            while (position < text.Length)
            {
                ...
                if (myTagType != null)
                {
                    ...
                    XmlNode myTagXmlNode = CreateTagXmlNode(myTag,fParsedDocument);
                ...
                else
                {
                    ...
                    parentNode.AppendChild(CreateTextXmlNode(myText, fParsedDocument));
		...
		/// </summary>
		protected XmlNode CreateTagXmlNode(TagBase tag,XmlDocument document)
		{
			#region Check the arguments
			...
			CheckXmlDocInitialized(fParsedDocument);

            XmlElement myTagNode = document.CreateElement(cTagXmlNodeName);

            XmlAttribute myTypeAttribute = document.CreateAttribute(cTagTypeXmlAttributeName);
			myTypeAttribute.Value = tag.Type;
			myTagNode.Attributes.Append(myTypeAttribute);

			if (tag.Value != null)
			{
                XmlAttribute myValueAttribute = document.CreateAttribute(cValueXmlAttributeName);
		...
		/// </summary>
		protected XmlNode CreateTextXmlNode(string text,XmlDocument document)
		{
			#region Check the arguments
            ...
            XmlElement myTextNode = document.CreateElement(cTextXmlNodeName);
		...
		/// </summary>
		protected void CheckXmlDocInitialized(XmlDocument document)
		{
            if (document == null)
        ...
        /// Checks whether the m_ParsedDocument is initialized.
        /// </summary>
        public bool IsDocumentInit
        {
            get
            {
                return (fParsedDocument == null) ? false : true;
            }
        }
		
        /// Returns the text string processed by the parser.
		/// </summary>
		public string ToText(XmlDocument document)
		{
			CheckXmlDocInitialized(document);

			StringBuilder myStringBuilder = new StringBuilder();
            bool useStringTags = (document == fParsedDocument) ? false : true;
            XmlNodesToText(myStringBuilder, document.ChildNodes[0].ChildNodes,useStringTags);
		...
		/// </summary>
		protected void XmlNodesToText(StringBuilder output, XmlNodeList nodes,bool useStringTags)
		{
            ...
            {
                XmlNodeToText(output, myNode,useStringTags);
		...
		/// </summary>
		protected void XmlNodeToText(StringBuilder output, XmlNode node,bool useStringTags)
		{
			#region Check arguments
			...
			{
				TagXmlNodeToText(output, node,useStringTags);
			}
		...
		/// </summary>
		private void TagXmlNodeToText(StringBuilder output, XmlNode node,bool useStringTags)
		{
			#region Check arguments
			...
			TagBase myTag = TagXmlNodeToTag(node,useStringTags);
			...
			if (node.ChildNodes.Count > 0)
				XmlNodesToText(output, node.ChildNodes,useStringTags);
		...
		/// </summary>
		protected TagBase TagXmlNodeToTag(XmlNode node,bool useStringTag)
		{
			#region Check arguments
            ...
            return GetTagFromType(myTagType, myTagValue, node.ChildNodes.Count > 0, useStringTag);
		}

		/// <summary>
		/// Retrurns the tag instance from its type and value.
		/// </summary>
        protected TagBase GetTagFromType(string type, string value, bool hasContents, bool useStringTag)
		{
            Type[] tags;
            if (useStringTag)
                tags = StringTags;
        ...
        /// <summary>
        /// The list of all available string tags.
        /// </summary>
        protected abstract Type[] StringTags { get; }

        /// <summary>
		/// Gets the parsed document in xml format.
		/// </summary>
		public XmlDocument ParsedDocument
		{
			get
			{
				CheckXmlDocInitialized(fParsedDocument);
